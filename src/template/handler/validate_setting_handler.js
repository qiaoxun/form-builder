...
var validate_number_decimal = function (controlInfo) {
    ...
    if (controlInfo.type === 'number' && controlInfo.decimalPlace < 0) {
        this.$message.error("Decimal Places can't be negative.");
...
var validate_static_datasource = function (controlInfo) {
    ...
    if (controlInfo.dataOptions.length <= 0) {
        this.$message.error("Static Data Source can't be empty, please add at least 1 record.");
        return false;
    }
    ...
    if (hasError) {
        this.$message.error("Static Data Source has empty item, please check and fix it.");
        return false;
    }
};
...
var validate_ajax_datasource = function (controlInfo) {
    ...
    if (_.isEmpty(controlInfo.ajaxDataUrl)) {
        ...
        this.$message.error("Ajax Data URL must not be empty.");
        return false;
    }
};
